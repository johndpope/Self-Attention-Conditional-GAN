...
class Generator(nn.Module):
    def __init__(self, z_dim,channels):
        ...
        self.model = nn.Sequential(
            nn.ConvTranspose2d(z_dim, 1024, 4, stride=1, bias=False),  # 4x4 (dense)
            nn.BatchNorm2d(1024),
            nn.ReLU(),

            nn.ConvTranspose2d(1024, 512, 4, stride=2, padding=(1, 1), bias=False),  # 2x
            nn.BatchNorm2d(512),
            nn.ReLU(),
            ...
            nn.ConvTranspose2d(64, channels, 3, stride=1, padding=(1, 1)),  # 1x (conv)
            nn.Tanh())
...
class Discriminator(nn.Module):
    def __init__(self, channels,leak=0.2):
        ...
        self.conv8 = SpectralNorm(nn.Conv2d(512, 512, 4, stride=2, padding=(1, 1)))  # x/2

        self.conv9 = SpectralNorm(nn.Conv2d(512, 1024, 3, stride=1, padding=(1, 1)))  # x
    ...
    def forward(self, input):
        ...
        m = nn.LeakyReLU(self.leak)(self.conv8(m))
        m = nn.LeakyReLU(self.leak)(self.conv9(m))

        return self.fc(m.view(-1, 4 * 4 * 1024))